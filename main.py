...
class BoxWorldWindow(pyglet.window.Window):
    ...
    def __init__(self, filename, **kwargs):
        ...
        self.fps_display = pyglet.window.FPSDisplay(self) # clock.ClockDisplay()

        clBlack = (0,0,0, 255)
        self.labels = {
            'Squares: Agent Hunters':  Label('', x=5, y=self.height-20, color=clBlack),
            'Circles: Target Hunters': Label('', x=300, y=self.height-20, color=clBlack),
        }
        self._update_label()
        ...
        # Set up agents
        self.world.setup_agents()
        self.world.setup_abilities()

    def _update_label(self, key=None, text='---'):
        if key == 'Squares: Agent Hunters' or key is None:
            self.labels['Squares: Agent Hunters'].text = 'Squares: Agent Hunters'
        if key == 'Circles: Target Hunters' or key is None:
            self.labels['Circles: Target Hunters'].text = 'Circles: Target Hunters'
